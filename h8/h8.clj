...
(defn exponent [n m]
  (cond
    (= m 0) 1
    (> m 0) (* n (exponent n (- m 1)))
    :else (throw (Exception. "Negative exponents are not supportedr."))

    ))

(defn exponent-tr [n m]
  (unwindrec exponent-tr-helper [n m collect]
    (= m 0) collect
    (> m 0)(exponent-tr-helper n (- m 1) (* collect n))
    (throw (Exception. "Negative exponents are not supported.")))
    (exponent-tr-helper n m 1))
...
(unwindrec sumlist-tr-helper [collect xs]
  (empty? xs) collect
  (not-empty xs) (sumlist-tr-helper (+ collect (first xs)) (rest xs))
...
(if (empty? xs)
  xs
(concat (first xs) (flattenlist (rest xs)))
))

(defn flattenlist-tr [xs]
(unwindrec flattenlist-tr-helper [collect xs]
  (empty? xs) collect
  (not-empty xs) (flattenlist-tr-helper (concat collect (first xs)) (rest xs))
  (throw (Exception. "Error."))
)
...
(if (empty? xs)
  '(())
(concat `(~xs) (postfixes (rest xs)))
))

(defn postfixes-tr [xs]
(unwindrec postfixes-tr-helper [collect xs]
  (empty? xs) (concat collect '(()))
  (not-empty xs) (postfixes-tr-helper (concat collect `(~xs)) (rest xs))
  (throw (Exception. "Error."))
)
